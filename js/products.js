  ...
  if(location.search.indexOf("kw=")!=-1){
    ...
    function loadPage(pno=0){
      $.ajax({
        ...
        success:function(output){
          ...
          var html="";
          for(var p of data){
            var {lid,shangping,price,title}=p;
            html+=`<div class="col-md-4 p-1">
              <div class="card mb-4 box-shadow pr-2 pl-2">
                <a>
                  <img class="card-img-top" src="${shangping}">
                </a>
                <div class="card-body p-0">
                  <h5 class="text-primary">￥${price.toFixed(2)}</h5>
                  <p class="card-text">
                    <a  class="text-muted small" title="${title}">${title}</a>
                  </p>
                  <div class="d-flex justify-content-between align-items-center p-2 pt-0">
                    ...
                    <button class="btn btn-outline-secondary p-0 border-0" type="button">+</button>
                    <a class="btn btn-primary float-right ml-1 pl-1 pr-1" href=" " data-lid="${lid}">立即购买</a>
                  </div>
                </div>
              </div>
            </div>`
    ...
    .on("click","li>a",function(e){
      ...
      if($a.parent().is(":not(.active,.disabled)")){
        var i= $("#plist+h6>nav>ul>li.active>a").html()-1;
        ...
        if($btn.is("button")){
          ...
          $.ajax({
            ...
            success:function(data){
              if(data.ok==0){
                alert("请先登录!");
                location.href="http://localhost:3000/login1.html?back="+location.href;
              }else{
                $.ajax({
